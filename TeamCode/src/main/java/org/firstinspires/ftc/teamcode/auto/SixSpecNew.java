package org.firstinspires.ftc.teamcode.auto;

import com.pedropathing.localization.Pose;
import com.qualcomm.robotcore.eventloop.opmode.Autonomous;
import com.qualcomm.robotcore.eventloop.opmode.OpMode;

import org.firstinspires.ftc.teamcode.util.dairy.Robot;
import org.firstinspires.ftc.teamcode.util.dairy.features.LoopTimes;
import org.firstinspires.ftc.teamcode.util.dairy.subsystems.Chassis;
import org.firstinspires.ftc.teamcode.util.dairy.subsystems.Intake;
import org.firstinspires.ftc.teamcode.util.dairy.subsystems.IntakeSlides;
import org.firstinspires.ftc.teamcode.util.dairy.subsystems.Outtake;
import org.firstinspires.ftc.teamcode.util.dairy.subsystems.OuttakeSlides;

import dev.frozenmilk.dairy.core.util.features.BulkRead;
import dev.frozenmilk.mercurial.Mercurial;
import dev.frozenmilk.mercurial.commands.Lambda;
import dev.frozenmilk.mercurial.commands.groups.Parallel;
import dev.frozenmilk.mercurial.commands.groups.Sequential;
import dev.frozenmilk.mercurial.commands.Command;
import dev.frozenmilk.mercurial.commands.util.Wait;

@Mercurial.Attach
@Chassis.Attach
@Outtake.Attach
@OuttakeSlides.Attach
@Intake.Attach
@IntakeSlides.Attach
@LoopTimes.Attach
@BulkRead.Attach
@Autonomous
public class SixSpecNew extends OpMode {
    public static Pose intakePose = new Pose(20, 40, 0);
    public static double[] intakeOffsetsY = {0, 0, 0, 0, 0};
    public static double[] intakeOffsetsX = {0, 0, 0, 0, 0};
    public static Pose outtakePose = new Pose(49, 74, 0);
    public static double[] outtakeOffsetsY = {10, 8, 6, 4, 2, 0};
    public static double[] outtakeOffsetsX = {0, 0, 0, 0, 0, 0};
    public static double outtakeStopOffset = -2;
    public static double intakeStopOffset = -2;

    Command Outtake(int i){
        return new Parallel(
                OuttakeSlides.runToPosition(OuttakeSlides.submersiblePos),
                new Wait(0.5).then(new Parallel(
                        Outtake.setArm(Outtake.armSubmersiblePos),
                        Outtake.setPivot(Outtake.pivotSubmersiblePos)
                )),
                new Sequential(
                        Chassis.setSloppy(),
                        Chassis.driveToPoint(
                                new Pose(
                                        outtakePose.getX() + outtakeOffsetsX[i] + outtakeStopOffset,
                                        outtakePose.getY() + outtakeOffsetsY[i],
                                        outtakePose.getHeading())
                        ),
                        Chassis.setExact(),
                        Chassis.setConstantDrivePower(1),
                        OuttakeSlides.runToPosition(OuttakeSlides.scoreSubmersiblePos),
                        Outtake.openClaw(),
                        Chassis.releaseConstantDrivePower()
                )
        );
    }

    Command Intake(int i){
        return new Parallel(
                new Wait(0.5).then(new Parallel(
                        Outtake.setArm(Outtake.armSpecPos),
                        Outtake.setPivot(Outtake.pivotSpecPos),
                        OuttakeSlides.runToPosition(OuttakeSlides.minPos)
                )),
                new Sequential(
                        Chassis.setClean(),
                        Chassis.driveToPoint(
                                new Pose(
                                        intakePose.getX() + intakeOffsetsX[i] + intakeStopOffset,
                                        intakePose.getY() + intakeOffsetsY[i],
                                        intakePose.getHeading())
                        ),
                        Chassis.setExact(),
                        Chassis.setConstantDrivePower(-1),
                        new Wait(0.125),
                        Outtake.closeClaw(),
                        Chassis.releaseConstantDrivePower()
                )
        );
    }

    Command PushSamps = new Sequential(
            Chassis.setSloppy(),
            Chassis.releaseHeading(),
            new Lambda("4th-root").setInit(() -> {
                Chassis.exponentialTransformHeading = .25;
                Chassis.exponentialTransformTranslational = .25;
            }),
            new Parallel(
                    Robot.setState(Robot.State.INTAKE_SPEC),
                    new Sequential(
                            new Wait(.5),
                            IntakeSlides.extend()
                    ),
                    new Sequential(
                            Chassis.driveToPoint(new Pose(32, 52, Math.toRadians(-90))),
                            Chassis.driveToPoint(new Pose(32, 43, Math.toRadians(-30))),
                            Chassis.driveToPoint(new Pose(28, 44, Math.toRadians(-90))),
                            Chassis.driveToPoint(new Pose(32, 33, Math.toRadians(-30))),
                            Chassis.driveToPoint(new Pose(28, 34, Math.toRadians(-90))),
                            Chassis.driveToPoint(new Pose(32, 33, Math.toRadians(-30))),
                            Chassis.driveToPoint(new Pose(28, 34, Math.toRadians(-90)))
                    )
            ),
            new Lambda("2nd-root").setInit(() -> {
                Chassis.exponentialTransformHeading = .5;
                Chassis.exponentialTransformTranslational = .5;
            }),
            Chassis.suppressHeading(),
            Chassis.setClean()
    );

    @Override
    public void init() {
        Robot.init();
        Chassis.holdPoint = true;
        Outtake.setClaw(Outtake.clawClosePos);
        Outtake.isClawOpen = false;
        Outtake.setPosition(Outtake.armInitPos);
        Outtake.setPivotManual(Outtake.pivotOuttakeSpec);
        IntakeSlides.setPowerManual(-0.3);
        Intake.setIntakeManual(Intake.hoverPos);

        Robot.stateMachine.setState(Robot.State.INTAKE_SPEC);
    }

    @Override
    public void start() {
        new Sequential(
                Outtake(0),
                PushSamps,
                new Parallel(
                        IntakeSlides.retract(),
                        Intake(0)
                ),
                Outtake(1),
                Intake(1),
                Outtake(2),
                Intake(2),
                Outtake(3),
                Intake(3),
                Outtake(4),
                Intake(4),
                Outtake(5)
        ).schedule();
    }

    @Override
    public void loop() {

    }
}
